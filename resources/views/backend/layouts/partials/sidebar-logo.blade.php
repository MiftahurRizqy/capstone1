<aside
    ...
    class="sidebar fixed left-0 top-0 z-999 flex h-screen w-[290px] flex-col overflow-y-hidden border-r {{ config('settings.sidebar_bg_lite') ? '' : 'bg-gray-800' }} px-5 border-gray-800 dark:bg-gray-900 lg:static lg:translate-x-0"
    id="appSidebar"
    x-data="{
        init() {
            this.updateBg();
    ...
    >
        <a href="{{ route('admin.dashboard') }}">
            <span class="logo" :class="sidebarToggle ? 'hidden' : ''">
                <img
                    class="dark:hidden"
                    src="{{ config('settings.site_logo_lite') ?? '/images/logo/logo.png' }}"
                    alt="Logo"
                />
                <img
                    class="hidden dark:block"
                    src="{{ config('settings.site_logo_dark') ?? '/images/logo/logo.png' }}"
                    alt="Logo"
                />
            </span>
            <img
                ...
                src="{{ config('settings.site_icon') ?? '/images/logo/kecil.png' }}"
                alt="Logo"
            />
        </a>
    </div>
