                        ...
                        <form action="{{ route('admin.jaringan.wilayah.store') }}" method="POST" class="space-y-4" x-data="{
                            ...
                            async fetchKabupaten(isInitialLoad = false) {
                                ...
                                if (this.selectedProvinsi) {
                                    ...
                                    try {
                                        const response = await fetch(`{{ route('api.wilayah.children') }}?parent_id=${this.selectedProvinsi}&child_type=kabupaten`);
                                        if (!response.ok) {
                            ...
                            async fetchKecamatan(isInitialLoad = false) {
                                ...
                                if (this.selectedKabupaten) {
                                    ...
                                    try {
                                        const response = await fetch(`{{ route('api.wilayah.children') }}?parent_id=${this.selectedKabupaten}&child_type=kecamatan`);
                                        if (!response.ok) {
                                            const errorText = await response.text();
                                            throw new Error(`Network response was not ok: ${response.status} - ${errorText}`);
                                        }
                            ...
                            async fetchKelurahan(isInitialLoad = false) {
                                ...
                                if (this.selectedKecamatan) {
                                    ...
                                    try {
                                        const response = await fetch(`{{ route('api.wilayah.children') }}?parent_id=${this.selectedKecamatan}&child_type=kelurahan`);
                                        if (!response.ok) {
                                            const errorText = await response.text();
                                            throw new Error(`Network response was not ok: ${response.status} - ${errorText}`);
                                        }
